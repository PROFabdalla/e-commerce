...
function goto(x){
    ...
    window.open(`product.html?id=${x}`,"_self");
            ...
            var return_products=JSON.parse(localStorage.getItem("products"));
                ...
                for(var i=0;i<return_products.length;i++){
                    name_of_products.push(return_products[i]['prod_name']+" "+ return_products[i]['prod_id']+" "+return_products[i]['image']);        
                }
                var searchdiv=document.createElement("div");
                searchdiv.classList.add("alert", "alert-primary");
                searchdiv.setAttribute("style","position: absolute;left:78%;top:73%;width:15%");
            ...
            function search(){
                ...
                var ides=[];   
                var images=[];            
                var searchullist=document.createElement("ul");
                var the_message=document.getElementById("search_id").value;
                if(the_message!=''){

                    for (const iterator of name_of_products) {
                        if(iterator.split(" ")[0].toLowerCase().includes(the_message)){
                            resualt.push(iterator.split(" ")[0]);
                            ides.push(iterator.split(" ")[1]);
                            images.push(iterator.split(" ")[2]);
                            }
                }
                if(resualt.length>0){
                for(var i=0;i<resualt.length;i++){
                    ...
                    image.setAttribute("style","margin-left :40px")               
                    ah.append(image);
                    ah.setAttribute("color","red");
                    ah.setAttribute("onclick",`gotoproductusingsearch(${ides[i]})`);
                    searchlist.append(ah);
                    searchullist.append(searchlist);
                    searchdiv.classList.add("d-block");
                }
                searchdiv.append(searchullist);
...
function gotoproductusingsearch(y){
